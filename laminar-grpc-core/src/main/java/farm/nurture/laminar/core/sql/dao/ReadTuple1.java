...
public class ReadTuple1 extends ReadBase<Object> {

    private static final Logger LOG = LoggerFactory.getLogger(ReadTuple1.class);
    List<Object> records = null;

    public ReadTuple1(final List<Object> records) {
        this.records = records;
    }

    public ReadTuple1() {}

    protected final List<Object> populate() throws SQLException {
        if (null == this.rs) {
    ...
    @Override
    protected final Object getFirstRow() throws SQLException {
        if (null == this.rs) {
            LOG.warn(RS_NOT_INITIALIZED_MSG);
            throw new SQLException(RS_NOT_INITIALIZED_MSG);
        }
